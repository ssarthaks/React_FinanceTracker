...
function Report() {
  ...
  const totalIncome = trans.reduce((sum, transaction) => sum + (transaction.income || 0), 0);
  const totalExpenses = trans.reduce((sum, transaction) => sum + (transaction.expense || 0), 0);

  return (
    <div className="p-6 max-w-3xl mx-auto bg-white rounded-xl shadow-md space-y-4">
      <h1 className="text-center font-bold text-indigo-600 text-xl">Summary Report</h1>
